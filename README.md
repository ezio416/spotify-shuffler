...
Switching devices when listening to music often has the annoying side effect that the same queue of songs that played on the last device also starts playing on the new one. Run this program on something always-on like a server and it will monitor device changes and re-shuffle your queue automatically. Initial setup should be done on a computer with a GUI (may be fixed later).
